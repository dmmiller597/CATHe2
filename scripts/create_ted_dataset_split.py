...
def mmseqs_search(
    ...
    run_cmd([
        ...
        "-c", str(0),               # still handle coverage filter post-hoc if desired
...
def filter_training_pool(
    ...
    mmseqs_search(db_pool, db_test, resdb1, tsv1, tmp_dir, mmseqs, sens, iters, cov_mode, pid_thresh)
    ids1 = extract_ids_above(parse_search_tsv(tsv1), pid_thresh)
    # search pool vs val
    tsv2 = tmp_dir/"pool_vs_val.tsv"; resdb2 = tmp_dir/"res2"
    mmseqs_search(db_pool, db_val, resdb2, tsv2, tmp_dir, mmseqs, sens, iters, cov_mode, pid_thresh)
...
def cluster_large_families(
    ...
    pid_cluster: float, cov_mode: int, cov_cluster: float
) -> pd.DataFrame:
    ...
    for sf, group in tqdm(df.groupby("SF"), desc="Clustering families"):
        if len(group) <= fam_thresh:
            ...
            mmseqs_cluster(fam_db, clu_db, tmp_dir, clu_tsv, mmseqs, pid_cluster, cov_mode, cov_cluster)
...
def build_lookup_eval(
    ...
    mmseqs: str, sens: float, iters: int, cov_mode: int, pid_thresh: float
) -> pd.DataFrame:
    ...
    return filter_training_pool(pool, test_df, pd.DataFrame(columns=all_df.columns),
                                tmp_dir, mmseqs, sens, iters, cov_mode, pid_thresh)
